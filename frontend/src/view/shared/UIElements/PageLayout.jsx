...
const PageLayout = ({ children, datas, data, columns, setSearchTerm, searchTerm }) => {
    ...
    return (
        <main className='flex flex-col md:flex-row justify-between items-start p-6 pt-20 gap-6'>

            <div className='flex flex-col w-full md:w-1/4'>
                <div className='flex items-center gap-6 mb-5'>
                    <div className='text-4xl font-medium text-gray-700'>Projects</div>
            ...
            <div className='flex flex-col w-full md:w-full gap-4'>
                <div className='flex justify-between items-center gap-3'>
                    ...
                    <div>
                        <label className='border border-gray-200 bg-gray-200 p-3 text-sm rounded-md px-3 text-black font-light'>
                            Show
                    ...
                    <div className='flex justify-between items-center border-l pl-3 border-gray-300'>
                        <button className='md:block hidden bg-blue-500 text-white py-2 px-4 rounded-lg'>
                            Add User
                        </button>
                ...
                <div className='flex-grow'>
                    <div className='max-h-[40%] overflow-x-auto'>
                        ...
                        <table className='min-w-full table-auto '>
                            {/* Table headers */}
                            <thead>
                                <tr className="bg-cyan-100 cursor-pointer">
                                    {columns.map((column, index) => (
                                        <th key={index} className={`py-3 px-4 text-gray-700 text-sm font-medium uppercase ${column.class}`} onClick={() => handleSort(column.sort)}>
                                            {column.title}
                            ...
                            <tbody className='bg-gray-200'>
                                {currentData.map((item) => (
                                    <tr className="hover:bg-sky-50 hover:border-gray-800 text-gray-700 m-6" key={item.id}>
                                        {columns.map((column, index) => (
                                            <td className={`py-1 my-3 text-sm font-light ${column.class}`} key={column.key}><div className={`py-3 bg-gray-200 px-4 border border-transparent h-[47px] items-center flex font-thin ${index == 0 ? 'rounded-l-lg' : ''} ${index == columns.length - 1 ? 'rounded-r-lg' : ''} ${column.key == 'action' ? '' : 'hover:border-gray-200 hover:bg-gray-100'}`}>{item[column.key]}</div></td>
                                        ))}
                                    </tr>
                                ))}
                            </tbody>
